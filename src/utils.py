...
def load_wav(vid_path, sr, mode='train'):
    ...
    tf_sr_ret = tf_sr_ret.eval(session=tf.compat.v1.Session())
    print("tf_wav={}, tf_sr_ret={}".format(tf_wav_np.shape, tf_sr_ret))
    assert sr_ret == sr
    #return tf_wav, tf_sr_ret
    if mode == 'train':
        ...
        if 0.1 < 0.3:
            ...
            tf_reverse_wav = tf.compat.v1.reverse(tf_extended_wav, axis=[0])
...
def lin_spectogram_from_wav(wav, hop_length, win_length, n_fft=1024):
    ...
    tf_linear = tf.signal.stft(signals=tf_wav, frame_length=win_length,
    ...
    # print("tflinear={}, linear={}".format(tflinear.shape, linear.shape))
    return tflinear
